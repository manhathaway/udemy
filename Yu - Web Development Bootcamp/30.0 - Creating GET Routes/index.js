...
app.get("/filter", (req, res) => {
  ...
  try {
    res.json(jokes.filter(x => x.jokeType == type));
  } catch (error) {
    res.send(error.message);
  };
});
...
app.post("/joke", (req, res) => {
  try {
    if (req.body.jokeText && req.body.jokeType) {
      ...
      res.send("Fill out the necessary fields: jokeText and jokeType.")
    };
  } catch (error) {
    res.send(error.message);
  };
...
app.patch("/joke/:id", (req, res) => {
  ...
  try {
    if (req.body.jokeText) {
      ...
      res.json(jokes[id - 1]);
    } else if (req.body.jokeType) {
      jokes[id - 1].jokeType = req.body.jokeType;
      res.json(jokes[id - 1]);
    } else {
      res.send("Modification required for PATCH request.")
    }
  } catch (error) {
    res.send(error.message);
  }
...
app.delete("/joke/:id", (req, res) => {
  const id = parseInt(req.params.id);
  try {
    jokes = jokes.filter(x => x.id != id);
    res.send(`Joke #${id} deleted successfully.`)
  } catch (error) {
    res.send(error.message);
  };
});
